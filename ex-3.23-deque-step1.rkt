...
;; STEP 1: ADD an abstraction barrier for each slot (previous called items)
...
(define (make-deque)
  (let ((front-ptr '()) ; would point to a slot
    ...
    (define (make-slot data) (mcons data '()))
    (define (get-data slot) (mcar slot))
    (define (get-next slot) (mcdr slot))
    (define (set-next! slot value) (set-mcdr! slot value))
